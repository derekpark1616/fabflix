	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (DB.dbcon == null) {
				...
				// String encodedURL =
				// response.encodeRedirectURL("empaccess.html");
				// response.sendRedirect(encodedURL);
			}
			// Create an execute an SQL statement to select all of table"Stars"
			// records

			String[] tables = new String[] { "movies", "creditcards", "customers", "stars", "genres", "stars_in_movies",
					"genres_in_movies", "sales", "employees" };
			int arSize;
			...
			for (int i = 0; i < arSize; i++) {
				...
				out.println("<tr>" + "<td>" + "Table " + dbTable + "</td>" + "<td>" + "has " + metadata.getColumnCount()
						+ " columns" + "</td>" + "</tr>");
				...
				for (int j = 1; j <= metadata.getColumnCount(); j++) {
					...
					// out.println("<tr>" +"<td>" +"Type of attribute '" +
					// metadata.getColumnName(j) + "' is " +
					// metadata.getColumnTypeName(j));
					out.println("<tr>" + "<td>" + metadata.getColumnName(j) + "</td>" + "<td>"
							+ metadata.getColumnTypeName(j) + "</td>" + "</tr>");
