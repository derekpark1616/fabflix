...
public class RegisterCreditCardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			PreparedStatement ccinsert = DB.dbcon.prepareStatement(
					"REPLACE INTO `creditcards` (`id`,`first_name`,`last_name`,`expiration`) VALUES (?,?,?,?)");
			ccinsert.setString(1, cCard);
			ccinsert.setString(2, fName);
			ccinsert.setString(3, lName);
			ccinsert.setDate(4, expirationDate);
			ccinsert.executeUpdate(); // Store into our Database the Credit Card
										// info
