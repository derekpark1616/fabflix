...
public class CustomerResgistrationServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			PreparedStatement customerinsert = DB.dbcon
					.prepareStatement("INSERT INTO `customers` (`first_name`,`last_name`,"
							+ "`cc_id`,`address`,`email`,`password`) VALUES (?,?,?,?,?,?)");

			PreparedStatement getCcard = DB.dbcon
					.prepareStatement("Select cc.id from creditcards as cc where cc.id = ? ");
			...
			if (cc.next()) {
				do {
					...
					customerinsert.executeUpdate(); // Store into our Database
													// the Star info
