...
public class DBInteraction {
	...
	public static void InsertMoreMv(int mvID, String mvTtle, String mvDir) {
		...
		try {
			...
			/*
			 * //dbconn = DatabaseConnection.getConnection(); String bigQ =
			 * "INSERT INTO `movies`(`id`,`title`,`year`,`director`) SELECT * FROM (SELECT ?,?,?,?) AS tmp WHERE NOT EXISTS (SELECT name FROM movies WHERE title = ? ) LIMIT 1;"
			 * ; PreparedStatement movieinsert =
			 * DB.dbcon.prepareStatement(bigQ);
			 * 
			 * movieinsert.setLong(1,mvID); movieinsert.setString(2,mvTtle);
			 * movieinsert.setLong(3,yr); movieinsert.setString(4,mvDir);
			 * movieinsert.setLong(5,mvID); retId = movieinsert.executeUpdate();
			 */

			PreparedStatement movieinsert = DB.dbcon
					.prepareStatement("INSERT IGNORE INTO `movies` (`id`,`title`,`year`,`director`) VALUES (?,?,?,?)");
	...
	public static void InsertStar(String f, String l, String date) throws ParseException {
		...
		try {
			...
			// dbconn = DatabaseConnection.getConnection();
			PreparedStatement starinsert = DB.dbcon
	...
	public static void InsertMovie(int i, String title, int year, String director) {
		...
		try {
			...
			/*
			 * String bigQ =
			 * "INSERT INTO movies(`id`,`title`,`year`,`director`) SELECT * FROM (SELECT ?,?,?,?) AS tmp WHERE NOT EXISTS (SELECT title FROM movies WHERE title = ?) LIMIT 1;"
			 * ; PreparedStatement movieinsert =
			 * DB.dbcon.prepareStatement(bigQ);
			 * 
			 * movieinsert.setLong(1,i); movieinsert.setString(2,title);
			 * movieinsert.setLong(3,year); movieinsert.setString(4,director);
			 * movieinsert.setString(5,title); retId =
			 * movieinsert.executeUpdate();
			 */
	...
	public static void InsertStarsInMovies() {
		...
		try {
			...
			// dbconn = DatabaseConnection.getConnection();
			PreparedStatement stareInMovieinsert = DB.dbcon
					.prepareStatement("INSERT INTO `stars_in_movies` (`star_id`,`movie_id`) VALUES (?,?)");
			PreparedStatement starinsert = DB.dbcon
					.prepareStatement("INSERT INTO `stars` (`first_name`,`last_name`) VALUES (?,?)");
			PreparedStatement getStarId = DB.dbcon
					.prepareStatement("SELECT * FROM stars WHERE first_name = ? AND last_name =?");

			for (Map.Entry<String, Integer> pair : SAXParserSGInMovies.MovieIdStageMap.entrySet()) { // For
																										// each
																										// genre
																										// in
																										// our
				...
				if (!STAR.contains(w)) {
					...
					starinsert.setString(1, sf); // need to split the stage name
													// here
					...
					/*
					 * // Find the genre Id in our database if(
					 * res_starId.next() ) { // If the term is in our databse,
					 * Long starId = res_starId.getLong(1);
					 * 
					 * stareInMovieinsert.setLong(1, starId);
					 * stareInMovieinsert.setLong(2, f);
	...
	public static void InsertGenre() {
		...
		try {
			...
			// dbconn = DatabaseConnection.getConnection();
			PreparedStatement genreInMovieinsert = DB.dbcon
			...
			for (Map.Entry<String, Integer> pair : SAXParserMovies.catMovieMap.entrySet()) { // For
																								// each
																								// genre
																								// in
																								// our
				...
				if (!go.contains(w)) {
					...
					/*
					 * // Find the genre Id in our database if(
					 * res_genreId.next() ) { // If the term is in our databse,
					 * genreId = res_genreId.getLong(1);
					 * 
					 * } res_genreId.close(); genreInMovieinsert.setLong(1,
					 * genreId); genreInMovieinsert.setLong(2, f);
					 * genreInMovieinsert.executeUpdate();
					 */
				}
