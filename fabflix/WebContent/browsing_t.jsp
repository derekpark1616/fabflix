...
<table border="1" cellpadding="5" cellspacing="5">
	...
	<tr>
		...
		<td style="border-right: solid;"><a
			href="/fabflix/movie.do?movieId=<%=movie.id%>">Click here to buy</a></td>
...
<c:if test="${currentPage != 1}">
	<td><a
		href="movie.ok?pg=${currentPage - 1}&nRows=${nRows}&lt=${ttle}">Previous</a></td>
</c:if>
	...
	<tr>
		<c:forEach begin="1" end="${noOfPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<td><a href="movie.ok?pg=${i}&nRows=${nRows}&lt=${ttle}">${i}</a></td>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	</tr>
...
<c:if test="${currentPage lt noOfPages}">
	<td><a
		href="movie.ok?pg=${currentPage + 1}&nRows=${nRows}&lt=${ttle}">Next</a></td>
</c:if>
