...
<table border="1" cellpadding="5" cellspacing="5" class="sortable">
	...
	<tr>
		<td style="border-right: solid;"><a
			href="/fabflix/movie.DBCPDo?movieId=<%=movie.id%>"><%=movie.id%></a></td>
		...
		<td style="border-right: solid;">
			<form id='myForm'>
				<input type='hidden' name='mvId' value='<%=movie.id%>' /> <br /> <input
					type='text' onmouseover="ajaxFunction('<%=movie.id%>');" size='60'
					value='<%=movie.title %>' />
			</form>
		</td>
		...
		<td style="border-right: solid;"><a
			href="/fabflix/movie.DBCPDo?movieId=<%=movie.id%>">Buy Now</a></td>
		<td style="border-right: solid;"><a
...
<c:if test="${currentPage != 1}">
	<td><a
		href="movie.doDBCP?pg=${currentPage - 1}&nRows=${nRows}&Title=${ttle}&Year=${Year}&Director=${Director}&stare=${stare}&pool=${pool}">Previous</a></td>
</c:if>
	...
	<tr>
		<c:forEach begin="1" end="${noOfPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<td><a
						href="movie.doDBCP?pg=${i}&nRows=${nRows}&Title=${ttle}&Year=${Year}&Director=${Director}&stare=${stare}&pool=${pool}">${i}</a></td>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	</tr>
...
<c:if test="${currentPage lt noOfPages}">
	<td><a
		href="movie.doDBCP?pg=${currentPage + 1}&nRows=${nRows}&Title=${ttle}&Year=${Year}&Director=${Director}&stare=${stare}&pool=${pool}">Next</a></td>
</c:if>
